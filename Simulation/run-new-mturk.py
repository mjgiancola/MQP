...
if __name__=='__main__':
  ...
  if args.m:
    fp = open("Tests/NewMTurk//MV_results.txt", 'w')
  elif args.n:
    fp = open("Tests/NewMTurk/BASE_results.txt", 'w')
  elif args.r:
    fp = open("Tests/NewMTurk/RSM_results.txt", 'w')
  else:
    fp = open("Tests/NewMTurk/DSM_results.txt", 'w')

  for sim in range(100): # Run 100 simulations
    data = init_from_file("Tests/NewMTurk/data/%d.txt" % sim, 1, not args.r, True)
